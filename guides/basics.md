...
| `series` | `%{column: nil, normalize: :local}` | Several series in one model, told apart by `column`, each scaled on its own or all together, see [Several series](series.md) |
